...
> 本分支直接将现有模块升级到 Trino 440（未拆分独立模块）。若后续恢复多版本段，可参考以上方案按版本段拆分。

### 版本与依赖
- Trino 版本：435 → **440**（`gradle/libs.versions.toml`）
- Toolchain：trino-connector 模块使用 **JDK 21**（classfile 65 对齐 Trino 440）
- Catalog 版本校验：`MIN/MAX_SUPPORT_TRINO_SPI_VERSION = 440`（`CatalogRegister`）

### API 适配
- `finishInsert` 采用 Trino 440 非弃用签名（新增 `List<ConnectorTableHandle> sourceTableHandles`，调用前 unwrap）
- 依赖保持 `io.trino:trino-spi/jdbc/memory/testing` 由版本管理继承到 440

### 文档同步
- `docs/trino-connector/requirements.md` 与 `configuration.md` 更新为 Trino-server-440

### 测试
- 已执行：`./gradlew :trino-connector:trino-connector:test -PskipITs`

### 后续注意
- 如需再次支持 435/478 等多版本，建议按方案拆分模块并恢复版本段校验逻辑的可配置化。


## Trino 455 升级简报（当前分支）

> 继续沿用单模块直接升级方式，未拆分版本段模块。

### 版本与依赖
- Trino 版本：440 → **455**（`gradle/libs.versions.toml`）
- Toolchain：trino-connector 模块使用 **JDK 22**（classfile 66 对齐 Trino 455）
- Catalog 校验：`MIN/MAX_SUPPORT_TRINO_SPI_VERSION = 455`（`CatalogRegister`）
- JaCoCo：对 trino-connector 模块禁用，因 0.8.9 不支持 classfile 66

### 行为适配
- `GravitinoConnectorFactory`：
  - 仅在 coordinator 初始化/启动 `CatalogRegister`，worker 节点跳过，避免缺失 `discovery.uri` 导致加载失败
  - 若未配置 `discovery.uri`，从当前节点 host:port 生成默认值
- `finishInsert` / `finishMerge`：采用 Trino 455 非弃用签名，unwrap `sourceTableHandles` 后转发
- 移除 `ConnectorSplit.getInfo()` 实现以匹配新 SPI

### 测试调整
- 使用 `setWorkerCount(1)` 创建 `DistributedQueryRunner`
- `testAlterTable`：避免列名冲突，重命名 `a -> d`，add column 改为 `assertUpdate`
- `createTestTable`：只 drop 当前表，保留 schema，避免多表互相影响
- 单测通过：`./gradlew :trino-connector:trino-connector:test -PskipITs`

### 后续建议
- 若需多版本共存，按方案拆分模块并让版本校验/工具链可配置化
- 持续关注 Trino 后续 SPI/ABI 变更，必要时做定向覆盖
